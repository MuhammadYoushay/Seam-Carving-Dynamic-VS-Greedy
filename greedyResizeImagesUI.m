function greedyResizeImagesUI()
% Create a figure
fig = figure('Position', [150, 150, 1000, 500]);

% Set title and heading
fig.Name = 'Seam Remover - Greedy';
titleText = uicontrol('Style', 'text', 'String', 'Seam Remover - Greedy', 'FontSize', 16, 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'Position', [750, 10, 200, 70]);
...
newHeight = size(originalImage, 1);
newWidth = size(originalImage, 2);
...
function resizeCallback(~, ~)
    ...
    if strcmp(reductionType, 'Width')
        ...
        % pixelReduction = pixelReduction - width;
        newWidth = newWidth - pixelReduction;
        resizedImage = greedySeamCarveWidth(originalImage,[newHeight,newWidth]);
    else
        newHeight = newHeight - pixelReduction;
        resizedImage = greedySeamCarveHeight(originalImage,[newHeight,newWidth]);
    end
...
%     %     [inputImage, ~] = decreaseWidthGreedy(inputImage, energyFunc);
%     % end
% 
% 
...
%         energyFunc = energyCalcFunc(inputImage);
%         [inputImage, ~] = decreaseHeightGreedy(inputImage, energyFunc);
%     end
% 
